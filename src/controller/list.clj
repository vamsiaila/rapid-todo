...
(defn add-item-to-list
  ...
  {:status 200
   :body {:_id "123456789" :value "shoot email" :priority "high" :isCompleted false}}
  )
...
(defn update-item-in-list
  ...
  {:status 200
   :body {:_id "123456789" :value "shoot email to support" :priority "medium" :isCompleted false}}
  )
...
(defn mark-item-in-list-as-complete
  ...
  ; step-3 return the updated item in response
  {:status 200
   :body {:_id "123456789" :value "shoot email to support" :priority "medium" :isCompleted true}}
  )
